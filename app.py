...
def crear_mapa_pendientes_simple(X, Y, pendiente_grid, gdf_original):
    ...
    if np.sum(valid_mask) > 0:
        ...
        """
        ax2.text(0.02, 0.98, stats_text, transform=ax2.transAxes, fontsize=9, verticalalignment='top', bbox=dict(boxstyle="round,pad=0.3", facecolor='white', alpha=0.8), color='black')
...
def mostrar_resultados_textura(gdf_analizado, cultivo, area_total):
    ...
    try:
        ...
        for idx, row in gdf_analizado.iterrows():
            ...
            gdf_analizado.iloc[[idx]].plot(ax=ax, color=color, edgecolor='white', linewidth=1.5)
            centroid = row.geometry.centroid
        ...
        ax.legend(handles=legend_elements, title='Texturas', loc='upper left', bbox_to_anchor=(1.05, 1), facecolor='#161c25', labelcolor='white')
        plt.tight_layout()
        buf = io.BytesIO()
        plt.savefig(buf, format='png', dpi=150, bbox_inches='tight', facecolor=fig.get_facecolor())
        buf.seek(0)
...
if uploaded_file:
    with st.spinner("🔄 Procesando parcela..."):
        try:
            ...
            if gdf is not None:
                ...
                if st.button("🚀 EJECUTAR ANÁLISIS INTELIGENTE", type="primary", use_container_width=True):
                    resultados = None
                    if analisis_tipo in ["FERTILIDAD ACTUAL", "RECOMENDACIONES NPK"]:
                        ...
                        if analisis_tipo == "ANÁLISIS DE TEXTURA":
                            ...
                            if resultados.get('df_power') is not None:
                                ...
                                def crear_grafico_personalizado(series, titulo, ylabel, color_linea):
                                    ...
                                    ax.set_xlabel("Fecha", fontsize=11, color='white')
                                ...
                                with tab_cosecha:
                                    ...
                                    fig_potencial = crear_mapa_potencial_cosecha(gdf_analizado, cultivo)
                                    st.pyplot(fig_potencial)
                                    ...
                                    # --- Paso 7: Descarga ---
                                    buf_mapa = io.BytesIO()
                                    plt.savefig(buf_mapa, format='png', dpi=150, bbox_inches='tight', facecolor=fig_potencial.get_facecolor())
                                    buf_mapa.seek(0)
                                    st.download_button(
                            ...
                            def crear_mapa_estatico(gdf, titulo, columna_valor, analisis_tipo, nutriente, cultivo, satelite):
                                try:
                                    ...
                                    for idx, row in gdf.iterrows():
                                        ...
                                        gdf.iloc[[idx]].plot(ax=ax, color=color, edgecolor='white', linewidth=1.5)
                                        centroid = row.geometry.centroid
    ...
    st.info("📤 **Sube tu archivo de parcela** para comenzar el análisis inteligente.")
